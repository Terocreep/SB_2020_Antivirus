...
type T_Coul is (rouge, turquoise, orange, rose, marron, bleu, violet, vert, jaune, blanc, vide, v);
...
---- Types pour les coordonées des pieces
type TR_Pos is record
  x : T_Lig;
  y : T_Col;
end record;
type TV_Pos is array(integer range <>) of TR_Pos;
...
function nbConfig(f : p_piece_io.file_type) return integer;
end p_virus;
